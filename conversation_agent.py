...
class OpenAIConversationAgent:
    ...
    def handle_turn(self, user_utterance: str, recommendations_context: dict = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Step 9: Add search query if needed but don't end conversation
            if action in ["SEARCH", "SEARCH_READY"]:
                enhanced_query = self.query_enhancer.build_enhanced_query(self.memory, "recommendation")
                
                complete_response.update({
                ...
                history_json_list = history_to_json(self.memory.history)
                reranker = TwoStageContextualRerankerJSON(api_key=api_key)
                result = reranker.rerank_with_context(all_docs, history_json_list)

                # Clean access to results
                top_docs = result["top_10_documents"]
                conditions = result["ranking_conditions"]
                qa_check = result["quality_assurance"]


                print('reranking agent been called')
                print(conditions , '\n')
                print(top_docs , '\n')
                print(qa_check , '\n')
